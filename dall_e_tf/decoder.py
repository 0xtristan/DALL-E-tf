...
def dvae_decoder(img_size=None, group_count=4, n_init=128, n_hid=256, n_blk_per_group=2, output_channels=3,
    ...
    for i in reversed(range(group_count)):
        ...
        if i > 0:
            x = UpSampling2D(2)(x)
    x = act_fn(x)
    # They use 2*output_channels for a logit-laplace output model
    # I added a sigmoid since I'm just using MSE loss for now
    x = Conv2D(output_channels, 1, activation='sigmoid', dtype=tf.float32)(x)
    return Model(x_in, x)